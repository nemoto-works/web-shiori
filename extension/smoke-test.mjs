...
assert.match(contentScriptJs, /SLACK_TIMESTAMP_PERMALINK_SELECTOR/);
assert.match(contentScriptJs, /c-timestamp/);
assert.match(contentScriptJs, /thread_ts=/);
assert.match(contentScriptJs, /findSlackTimestampPermalinkFromElement/);
assert.match(contentScriptJs, /timestampPermalink/);
...
assert.match(popupHtml, /data-filter="active">未完了/);
assert.match(popupHtml, /data-filter="completed">完了/);
assert.match(popupHtml, /data-filter="all">すべて/);
assert.match(popupHtml, /aria-selected="true" data-filter="active"/);
assert.match(popupHtml, /note-item--completed\{[^}]*box-shadow:none;opacity:\.74/s);
...
assert.match(popupJs, /currentNoteFilter = 'active'/);
assert.match(popupJs, /TAB_LABELS/);
assert.match(popupJs, /splitNotesByStatus/);
assert.match(popupJs, /getVisibleNotesByFilter/);
assert.match(popupJs, /updateTabLabels/);
assert.match(popupJs, /notes\.filter\(\(note\) => !note\.completed\)/);
assert.match(popupJs, /notes\.filter\(\(note\) => note\.completed\)/);
assert.match(popupJs, /\[\.\.\.notesByStatus\.active, \.\.\.notesByStatus\.completed\]/);
assert.match(popupJs, /querySelectorAll\('\.note-tab'\)/);
assert.match(popupJs, /tab\.dataset\.filter/);
