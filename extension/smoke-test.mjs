...
assert.match(contentScriptJs, /getSlackCurrentWorkspaceMessageUrl/);
assert.match(contentScriptJs, /\/client\/\$\{clientWorkspace\}\/\$\{channel\}\/thread\/\$\{channel\}-\$\{timestampId\}/);
assert.match(contentScriptJs, /currentUrl\.hostname !== 'app\.slack\.com'/);
assert.doesNotMatch(contentScriptJs, /window\.location\.origin\)\.href/);
